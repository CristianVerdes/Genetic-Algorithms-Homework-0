...
def six_function (parameters = []):
    for number in parameters:
        return (4 - 2.1 * pow(number, 2) + float((number ** 4)/3)) * pow(number,2) + \
               number * number + (-4 + 4 * pow(number, 2)) * pow(number, 2)
...
def tema0 ():
    ...
    for repetitie in range(numar_rulari):
        ...
        lista_rezultate.append(six_function(parameters))
        # Mai sus schimbam functia folosita.

    print("Cea mai buna solutie: ", min(lista_rezultate))
    print("Cea mai proasta solutie: ", max(lista_rezultate))
